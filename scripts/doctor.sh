...
MISSION="Gateway Simulation: Operation First Contact"
...
if [ "${SDC_RUNNING:-0}" -gt 0 ]; then
    ...
    warn "SDC lab containers already running: ${THIS_LAB}" "If these belong to another mission, run 'make destroy' there first — this lab uses ports 2231-2233"
else
    PORTS_BUSY=""
    for port in 2231 2232 2233; do
    ...
    if [ -n "$PORTS_BUSY" ]; then
        fail "Ports 2231-2233 free" "Port(s)${PORTS_BUSY} in use — another mission lab or service is listening. Run 'make destroy' in the other mission, or free the port"
    else
        pass "Ports 2231-2233 free"
    fi
fi
